...
class MDSThrasher:
  ...
  def do_thrash(self):
    ...
    while not self.stopping:
      ...
      while True:
        status = self.manager.get_mds_status_by_rank(active_rank)
        if 'laggy_since' in status:
            last_laggy_since = status['laggy_since']
            break
        self.log('waiting till mds map indicates mds.{_id} is laggy/crashed'.format(_id=active_mds))
        time.sleep(2)
      ...
      self.manager.revive_mds_by_rank(active_rank, standby_for_rank=takeover_rank)

      status = {}
      while True:
        status = self.manager.get_mds_status_by_rank(active_rank)
        if status['state'] == 'up:standby' or status['state'] == 'up:standby-replay':
            break
        self.log('waiting till mds map indicates mds.{_id} is in replay or active'.format(_id=active_mds))
        time.sleep(2)
...
def task(ctx, config):
  ...
  for (active,standbys) in failure_groups.iteritems():
      ...
      weight = 1.0
      active_name = statuses_by_rank[active]['name']
      if 'thrash_weights' in config:
          weight = int(config['thrash_weights'].get('mds.{_id}'.format(_id=active_name), '0.0'))

      failure_group = [active_name]
